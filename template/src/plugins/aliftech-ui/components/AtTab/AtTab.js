...
const renderIcon = (icon, currentTab) => {
  ...
  return h(Icon, {
    class: [
      '--ml-0.5 mr-2 h-5 w-5',
      currentTab ? 'text-' + uiConfig.primaryTextColor + '-500' : 'text-gray-400 group-hover:text-gray-500',
    ],
    'aria-hidden': true,
  });
};
    ...
    'a',
    {
      class: [
        ...
        props.selected === props.item.value
          ? 'border-' + uiConfig.primaryBorderColor + '-500 text-' + uiConfig.primaryTextColor + '-600'
