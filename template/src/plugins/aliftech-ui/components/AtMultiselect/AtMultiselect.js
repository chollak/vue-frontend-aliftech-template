...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    let sortedOptions = computed(() => {
      return options.value.filter(option => option[props.valuePrint].includes(searchText.value));
    });
              ...
              this.selectedOptions.length
                ? this.selectedOptions.map((option, optionIdx) =>
                    h(
                      'button',
                      {
                        class: ['h-5 px-1 text-xs flex items-center rounded-lg', 'bg-' + option?.color + '-100'],
                      },
                      [
                        option?.title,
                        h(XIcon, {
                          class: 'h-3 w-3 ml-1',
                          onClick: () => this.removeElementFromSelectedOptions(optionIdx),
                        }),
                      ]
                    )
                  )
